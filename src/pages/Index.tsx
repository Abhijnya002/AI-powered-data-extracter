...
const Index = () => {
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      setProgress(100);
      toast({ title: "Processing complete!", description: "Download started." });
    } catch (err) {
      setError("An error occurred during processing.");
      toast({ title: "Processing error", description: "Something went wrong", variant: "destructive" });
    } finally {
      setProcessing(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4 bg-gradient-to-br from-purple-50 to-blue-50">
      <div className="text-center mb-12">
        <h1 className="text-4xl font-bold bg-gradient-to-r from-purple-600 to-blue-600 text-transparent bg-clip-text">
          Document Processing Suite
        </h1>
        <p className="mt-4 text-lg text-gray-700 dark:text-gray-300">{typewriterText}</p>
      </div>

      <Card className="w-full max-w-3xl p-8 shadow-xl bg-white">
        <CardHeader>
          <CardTitle className="text-xl font-semibold">Upload your Word document</CardTitle>
        ...
        <CardContent className="space-y-6">
          ...
          {!file ? (
            <div className="flex flex-col items-center gap-4">
              ...
              <Button onClick={() => fileInputRef.current?.click()}>Choose .docx file</Button>
            </div>
          ) : (
          ...
          {processedFile && !processing && (
            <div className="text-center mt-4">
              <CheckCircle className="inline h-6 w-6 text-green-500 mr-2" />
              <span className="font-medium text-green-700">{processedFile} is ready!</span>
            </div>
          )}
        </CardContent>
      </Card>
